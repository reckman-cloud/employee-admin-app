...
Configure Mosyle with `MOSYLE_API_TOKEN`, `MOSYLE_API_EMAIL`, and `MOSYLE_API_PASSWORD`. The function first sends the API access token as the `accessToken` header and the email/password credentials to Mosyle's `/login` endpoint. It then uses the returned bearer token to search `/devices`. `MOSYLE_API_URL` can override the default `https://businessapi.mosyle.com/v1` base URL.
