...
Configure Mosyle with `MOSYLE_API_TOKEN`, `MOSYLE_API_EMAIL`, and `MOSYLE_API_PASSWORD`. The function first sends the API access token as the `accessToken` header and the email/password credentials to Mosyle's `/login` endpoint. It reads the bearer token from the login response's `Authorization` header, then sends both the original `accessToken` and `Authorization: Bearer <token>` headers when searching `/devices` (with response-body token fields supported as a fallback). `MOSYLE_API_URL` can override the default `https://businessapi.mosyle.com/v1` base URL.
