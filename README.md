...
Configure Mosyle with `MOSYLE_API_TOKEN`; the function sends this value as Mosyle's required `accessToken` request field. `MOSYLE_API_URL` can override the default `https://managerapi.mosyle.com/v2/listdevices` endpoint when a tenant uses a different Mosyle API host.
