...
Configure Mosyle with `MOSYLE_API_TOKEN`, `MOSYLE_API_EMAIL`, and `MOSYLE_API_PASSWORD`. The function first sends the API access token as the `accessToken` header and the email/password credentials to Mosyle's `/login` endpoint. It reads the bearer token from the login response's `Authorization` header, then sends both the original `accessToken` and `Authorization: Bearer <token>` headers when posting a `list` operation to `/devices` (with response-body token fields supported as a fallback). The list operation is included in both the query string and JSON body for compatibility with Mosyle's operation dispatcher. `MOSYLE_API_URL` can override the default `https://businessapi.mosyle.com/v1` base URL.
