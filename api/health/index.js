...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (!exists) {
      context.res = { status: 503, headers: { "Content-Type":"application/json; charset=utf-8", ...cors }, body: { ok:false, reason:'queue-not-found' } };
      return;
    }
    ...
    context.log.error('Health check failed', reason, error?.message || error);
    context.res = { status: 503, headers: { "Content-Type":"application/json; charset=utf-8", ...cors }, body: { ok:false, reason } };
  }
};
