  ...
  return (
    <div id="entries-wrap">
      <table aria-describedby="entries-count">
        <thead><tr><th>First</th><th>Last</th><th>Title</th><th>Department</th><th>Business Unit</th><th>Manager</th><th style={{ width: 180 }}>Actions</th></tr></thead>
        <tbody>{entries.map(e => (
          <tr key={e.id}>
...
export default function FormApp() {
  ...
  const unsubmitted = useMemo(() => entries.filter(e => !e._meta?.submittedAt).length, [entries]); // kept for button disabled
  ...
  const saveEntry = e => {
    ...
    setEntries(prev =>
      form.id
        ? prev.map(x => (x.id === form.id ? { ...form, _meta: { ...(x._meta || {}), savedAt: now, schema: 2 } } : x))
        : [...prev, { ...form, id: uuid(), _meta: { savedAt: now, submittedAt: null, schema: 2 } }]
    );
    resetForm();
    showToast('Saved.');
  };
  ...
  const editEntry = id => {
    ...
    if (!x) return;
    setForm({ id: x.id, firstName: x.firstName, lastName: x.lastName, title: x.title, department: x.department, businessUnit: x.businessUnit, managerId: x.managerId, managerUpn: x.managerUpn || '', managerName: x.managerName || '' });
  };
  ...
  // Strip managerName; ensure managerUpn present (backfill from lists).
  const buildPayload = (listManagers, items) => items.map((e) => {
    ...
    if (!rest.managerUpn && rest.managerId) {
      ...
      if (m?.upn) rest.managerUpn = m.upn; // backfill older entries
    }
    return rest;
  });
  ...
  const submitAll = async () => {
    const toSubmit = entries; // submit all saved entries
    ...
    // Delete accepted entries; keep failures
        ...
        <form onSubmit={saveEntry} onReset={resetForm} noValidate>
          <div className="row">
            ...
            <ManagerComboBox
              ...
              onSelect={m => m ? setForm(f => ({ ...f, managerId: m.id, managerUpn: m.upn || '', managerName: m.name })) : setForm(f => ({ ...f, managerId: '', managerUpn: '', managerName: '' }))}
      ...
      <section className="card" style={{ gridColumn: '1 / -1' }} aria-labelledby="entries-title">
        ...
        <EntriesTable
          ...
          onEdit={id => {
            ...
            setForm({ id: e.id, firstName: e.firstName, lastName: e.lastName, title: e.title, department: e.department, businessUnit: e.businessUnit, managerId: e.managerId, managerUpn: e.managerUpn || '', managerName: e.managerName || '' });
          }}
